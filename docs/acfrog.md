...
#### Figure 3D — standalone truncation-index distributions

Panel 3D on its own (the per-bin fraction `p` histograms), one per filter state — this is
the panel where the tolerant-vs-sensitive tail difference reads most directly. Each figure's
title states its filter state; both use the Fig-2A OE groups with `itsn-1`/`adr-2` excluded.

Filtered (`> -1` exclusion — the default view):

![Figure 3D, filtered](acfrog_figure3d_worm.png)

No-filter comparison:

![Figure 3D, no filter](acfrog_figure3d_worm_nofilter.png)

In both, the **blue OE-tolerant curve has no tail beyond TI ≈ 0.2**, while the **red
OE-sensitive curve extends to ≈ 0.6** — the qualitative ceiling result. Generated by
`figure3d(exclude=True/False)`.
