...
`ACFROG~1.PDF` is the Pierce-lab NIH-style grant proposal ("OE threshold / Censoring Hypothesis" for DS & AD genes) whose preliminary figures this whole toolkit supports. `acfrog.md` is its markdown conversion (text reconstructed from the PDF; Figs 1 & 2 are conceptual diagrams/tables and are only described). **`regenerate_acfrog_figures.py`** regenerates the two *data* figures **faithfully from repo data** via `BhuvanFitter` → `acfrog_figure3_worm.png` (Fig 3, worm: `mco` OE-sensitive vs background truncation index) and `acfrog_figure4_cerebellum.png` (Fig 4, cerebellum: Hsa21 & triplication-sensitive gene distributions). **Honesty caveat baked into the script and the md:** the current data + bounded `truncationindex` do **not** reproduce the proposal's strong ceiling separation — worm `mco` vs background is Mann-Whitney n.s. (p ≈ 0.36, vs the proposal's p < 0.004), and the cerebellum triplication genes have truncation index ≈ 0 (consistent with the section-5 ρ ≈ −0.01 result). Run: `python regenerate_acfrog_figures.py`.
