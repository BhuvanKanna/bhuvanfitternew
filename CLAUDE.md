...
- **Validity filter over the OPEN interval `0 < truncationindex < 1`** (`valid_open`: `fit_success & 0 < TI < 1 & n_obs >= 30`) — **omits** the over-represented TI==0 (uncapped) and TI==1 (fully-capped) piles at the user's request (they are likely partly artefactual). This now matches the notebooks' `select`/`valid`. (An earlier version used `valid_incl0` / `0 <= TI <= 1` to keep TI==0 and count the grant's "uncapped fraction"; dropping the 0/1 piles removes that readout — notably the human GRANT/Hsa21 curve collapses to n=4 since most Hsa21 genes were uncapped.) Human genes are keyed ENSG→symbol via the GTEx source `Name`/`Description` map (`_cereb_table`), as the cerebellum notebook does.
- **Five figures** (`__main__` writes all): `grant_figure3b_{worm,human}.png` (**POS vs TOL only**: jittered dots + a black mean±SEM marker per group, Mann-Whitney U), `grant_figure3d_{worm,human}.png` (per-bin-fraction `p` histograms, POS/GRANT/TOL vs ALL), `grant_figure10a_human.png` (cumulative/CDF curves + KS vs ALL; cerebellum-only, as in the grant). `groups_for(dataset)` (`"worm"`/`"human"`) assembles the four TI arrays and prints a coverage line (n, median, mean) per group.
- **Result (faithful method, honest effect size; open-interval filter).** With TI∈(0,1), medians are close and no comparison is significant. **Human cerebellum** (n: POS 19, GRANT 4, TOL 168, ALL 13644): POS median 0.015 ≳ TOL 0.013 ≳ ALL 0.009, MWU POS-vs-TOL p ≈ 0.70 (n.s.); the human GRANT/Hsa21 curve is n=4 (most Hsa21 genes were uncapped and dropped). **Worm** (n: POS 36, GRANT 28, TOL 140, ALL 11062): POS median 0.057 ≳ TOL 0.041 ≈ ALL 0.044, with GRANT mean elevated by a few high-TI genes (mean 0.133, median 0.037); all worm MWU p n.s. The grant's strong separation is **not** reproduced with these broader, symbol-matched gene sets on the open-interval TI.
