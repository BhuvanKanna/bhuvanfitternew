...
- **Fig 3 OE groups come from Figure 2A, not `genes_of_interest.json`.** The script hard-codes `SENS_GENES` (24 OE-sensitive) / `TOL_GENES` (23 OE-tolerant) worm gene names extracted from Fig 2A's mcOE-"Any" shaded cells (the json lacks the OE-tolerant no-phenotype genes, which are the correct "absent" control for panel D). `build_worm_groups()` resolves those names → fourparam-table transcript IDs (`w{n}_{transcript}`) via the `Supplementary Data 1 trunc 20250702.xlsx` `GeneName`/`transcript`/`wwww` columns (hlh-34, sod-5, H39E23.3 aren't in the dataset and drop out).
...
- **`figure3b_comparison()`** → `acfrog_figure3b_comparison.png`: side-by-side of panel 3B as (left) a box plot over all transcripts (median, Mann-Whitney p ≈ 0.24) vs (right) the **grant's style** — one canonical transcript per gene, mean ± SEM, gene-name labels, Welch t-test p ≈ 0.29. Same metric/data/groups; the grant's stronger *p < 0.004* is **not** reproduced. Diagnosis (also in acfrog.md): the box-vs-grant difference is **transcript selection + mean-vs-median statistic**, NOT the `> -1` filter (no-filter barely moves p) and NOT the truncationindex formula (current metric ≈ the grant's own supplementary `trunc` for most genes: cle-1/dnsn-1/dip-2/Y54E10A.11/Y105E8A.1 all within ~0.05). Even the grant's own `trunc` on these 23/21 gene lists only reaches p ≈ 0.03–0.07, so the published p likely used an expanded gene set (+13 independent-study genes).
