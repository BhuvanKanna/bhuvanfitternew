...
- **Result.** Winner = **logistic regression, C=0.01** (heavy regularization): **CV/OOF ROC-AUC ≈ 0.82, PR-AUC ≈ 0.39–0.45** (≈5× the 0.082 baseline). **The signal is expression *level*, not distribution *shape*.** Ablation (`ablation_dosage_features.py`): `mean`-only ROC-AUC **0.824** ties/beats the full model (0.816), `top3_level` (mean/min/max) 0.820, `level_only` 0.818, `shape_only` only 0.712, and **`truncationindex`-only 0.528 (≈chance)**; adding shape to level does not help. So the classifier is essentially a "cerebellum mean-expression" detector — POS genes are more highly expressed in cerebellum than the TOL controls — which corroborates the repo's earlier finding that expression-level features (ρ≈0.43) carry the pTriplo signal while the truncation index does not (ρ≈−0.01). **Caveat:** this makes the score partly a tissue-expression confound (the genome-wide top hits are dominated by very-high-expression MT-\* genes), not evidence that distribution *shape* encodes dosage sensitivity.
