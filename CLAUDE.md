...
- **Validity filter deliberately KEEPS `truncationindex == 0`** (`valid_incl0`: `fit_success & 0 <= TI <= 1 & n_obs >= 30`) — the uncapped (TI=0) genes carry the whole tolerant-vs-sensitive signal and the grant explicitly counts them (its "63% have no ceiling", "0.70 vs 0.45 uncapped"). This is **not** the notebooks' `select`/`valid` (`0 < TI < 1`); do not swap them. Human genes are keyed ENSG→symbol via the GTEx source `Name`/`Description` map (`_cereb_table`), as the cerebellum notebook does.
...
- **Result (faithful method, honest effect size).** The grant's qualitative direction shows up **split across the two organisms** but neither reaches strong significance. In **human cerebellum**, POS is the most-truncated group (`frac0` 0.55 vs ALL 0.67), the grant's predicted direction, but MWU POS-vs-TOL p ≈ 0.41 (n.s.) and KS-vs-ALL p ≈ 0.29; GRANT/Hsa21 genes are the *least* truncated (`frac0` 0.85; MWU GRANT-vs-TOL p ≈ 0.006 in the "wrong" direction), consistent with the repo's prior ρ ≈ −0.01 pTriplo↔TI finding. In **worm**, GRANT is the most-truncated group (`frac0` 0.49 vs ALL 0.57) — matching the grant's original worm claim — while POS ≈ ALL; all worm MWU p > 0.23 (small n: POS 87, GRANT 55, TOL 324). The grant's strong separation is **not** reproduced with these broader, symbol-matched gene sets.
