...
- **`figure3b_comparison()`** → `acfrog_figure3b_comparison.png`: side-by-side of panel 3B as (left) a box plot over all transcripts (median, Mann-Whitney p ≈ 0.22) vs (right) the **grant's style** — one canonical transcript per gene, mean ± SEM, gene-name labels, Welch t-test p ≈ 0.11 (both after excluding itsn-1/adr-2; dropping those two elevated tolerant genes moved the grant-style p from ≈ 0.29 → ≈ 0.11). Same metric/data/groups; the grant's stronger *p < 0.004* is **not** reproduced. Diagnosis (also in acfrog.md): the box-vs-grant difference is **transcript selection + mean-vs-median statistic**, NOT the `> -1` filter (no-filter barely moves p) and NOT the truncationindex formula (current metric ≈ the grant's own supplementary `trunc` for most genes: cle-1/dnsn-1/dip-2/Y54E10A.11/Y105E8A.1 all within ~0.05). Even the grant's own `trunc` on these 23/21 gene lists only reaches p ≈ 0.03–0.07, so the published p likely used an expanded gene set (+13 independent-study genes).
